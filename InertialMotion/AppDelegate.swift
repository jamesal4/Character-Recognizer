...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func appendTrainingLog(entry: String?) {
        ...
        guard let documentDirectoryURL = urls.first else {
            print("Couldn't find Documents directory.")
            return
        }
        
        let fileURL = documentDirectoryURL.appendingPathComponent("trainingData.txt", isDirectory: false)
        do {
        ...
        guard let fileHandle = try? FileHandle(forUpdating: fileURL),
            let data = entry.data(using: .utf8) else {
